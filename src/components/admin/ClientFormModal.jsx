...
import { createClient, updateClient } from '../../utils/blogStore';
import { useAuth } from '../../contexts/AuthContext';

export const ClientFormModal = ({ isOpen, onClose, onSuccess, initialData }) => {
  const { token } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing && initialData._id) {
        await updateClient(initialData._id, formData, token);
      } else {
        ...
        >
          <div className="flex justify-between items-center p-6 border-b border-gray-200 dark:border-gray-800">
            <h2 className="text-xl font-bold font-space text-gray-900 dark:text-white">
              {isEditing ? 'Edit Client Company' : 'Add Client Company'}
            </h2>
            <button
              type="button"
              onClick={onClose}
